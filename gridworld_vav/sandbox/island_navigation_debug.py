...
world = gw.create_safety_island_world_nowalls()
...
island_colors = ['black','tab:green','white','tab:blue','tab:red','tab:gray','tab:green','tab:purple', 'tab:orange',  'tab:cyan']

mdp_plot.plot_optimal_policy_vav(opt_policy, world.features, walls=True, show=False, arrow_color='k',
    feature_colors=island_colors,filename=os.path.join(project_path,'figs/island/optimal.png'))
...
for test in tester.tests:
    q_indx = np.random.randint(len(test))
    question = test[q_indx]
    #find something that starts in initial state if possible
    (s,worse), (s,better) = question
    random_test.append((s,better))
...
mdp_plot.plot_test_questions(random_test, world.features, walls=True, show=False, arrow_color='k',
    feature_colors=island_colors,filename=os.path.join(project_path,'figs/island/arb-bb.png'))

###arp-pref
tester = vav.TrajectoryRankingBasedTester(world, precision, rollout_length, debug, use_suboptimal_rankings=False)
size_verification_test = tester.get_size_verification_test()
print("number of questions", size_verification_test)
...
for i, question in enumerate(tester.test):
    mdp_plot.plot_preference_query(question.traj_better, question.traj_worse, world.features, walls=True, show=False, 
        good_arrow_color='k', bad_arrow_color='orange', feature_colors=island_colors,
        filename=os.path.join(project_path,'figs/island/pref_{}.png'.format(i)))
...
mdp_plot.plot_test_questions(scot_question_list, world.features, walls=True, show=False, arrow_color='k',
    feature_colors=island_colors,filename=os.path.join(project_path,'figs/island/scot.png'))
...
mdp_plot.plot_test_questions(cs_question_list, world.features, walls=True, show=False, arrow_color='k',
    feature_colors=island_colors,filename=os.path.join(project_path,'figs/island/cs-10.png'))
